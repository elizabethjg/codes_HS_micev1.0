...
def corner_plot(samp,RIN,ROUT,relax=True,
    ...
    mmas = [lMNFW,cNFW,lMEin,cEin,alpha,lMfit,cfit]
    ...
    fr=open(folder+'../'+'allres_'+fname,'a')
    ...
    for x in mmas[:-1]:
        fm.write('$'+str('%.3f' % (x[1]))+'_{-'+str('%.3f' % (np.diff(x)[0]))+'}^{+'+str('%.3f' % (np.diff(x)[1]))+'}$ & ')
        fr.write(str('%.3f' % (x[1]))+'  '+str('%.3f' % (np.diff(x)[0]))+'  '+str('%.3f' % (np.diff(x)[1]))+'  ')
    x = mmas[-1]
    fr.write(str('%.3f' % (x[1]))+'  '+str('%.3f' % (np.diff(x)[0]))+'  '+str('%.3f' % (np.diff(x)[1]))+'  ')
    fm.write('$'+str('%.3f' % (x[1]))+'_{-'+str('%.3f' % (np.diff(x)[0]))+'}^{+'+str('%.3f' % (np.diff(x)[1]))+r'}$ \\'+' \n')
    fm.close()
    
    for x in qres[:-1]:
        fq.write('$'+str('%.3f' % (x[1]))+'_{-'+str('%.3f' % (np.diff(x)[0]))+'}^{+'+str('%.3f' % (np.diff(x)[1]))+'}$ & ')
        fr.write(str('%.3f' % (x[1]))+'  '+str('%.3f' % (np.diff(x)[0]))+'  '+str('%.3f' % (np.diff(x)[1]))+'  ')
    x = qres[-1]
    fq.write('$'+str('%.3f' % (x[1]))+'_{-'+str('%.3f' % (np.diff(x)[0]))+'}^{+'+str('%.3f' % (np.diff(x)[1]))+r'}$ \\'+' \n')
    fr.write(str('%.3f' % (x[1]))+'  '+str('%.3f' % (np.diff(x)[0]))+'  '+str('%.3f' % (np.diff(x)[1]))+'  ')
    fq.close()
...
def bias_model(hsamples,RIN=250,make_tab=False,fname='ltab'):
    ...
    for j in range(len(hsamples)):
        ...
        qwm += [fits.open(folder+'profile_'+samp+'.fits')[0].header['q2d_mean']]

        
        # 1 halo
        
        fstd = fits.open(folder+'fitresults_onlyq_'+RIN+'_'+ROUT[j]+'_profile_'+samp+'.fits')[1].data
        q1h  += [np.median(fstd.q[1500:])]
        eq1h += [np.diff(np.percentile(fstd.q[1500:], [16,50,84]))]

        if make_tab:

            fred = fits.open(folder+'fitresults_onlyq_'+RIN+'_'+ROUT[j]+'_reduced_profile_'+samp+'.fits')[1].data
            
            
            qres = [np.percentile(fstd.lM200[1500:], [16,50,84]),
                    np.percentile(fstd.c200[1500:], [16,50,84]),
                    np.percentile(fstd.q[1500:], [16,50,84]),
                    np.percentile(fred.q[1500:], [16,50,84])]
    
            
            fq=open(folder+'../'+'res_1h_'+fname+'.out','a')
            fq.write(hsamples[j]+' & ')
        ...
        qwoc  += [np.median(fstd.q[1500:])]
        eqwoc += [np.diff(np.percentile(fstd.q[1500:], [16,50,84]))]
        
        if make_tab:
            fred = fits.open(folder+'fitresults_2h_2q_woc_'+RIN+'_5000_reduced_profile_'+samp+'.fits')[1].data
            c200 = fits.open(folder+'fitresults_2h_2q_woc_'+RIN+'_5000_profile_'+samp+'.fits')[0].header['c200']
            
            qres = [np.percentile(fstd.lM200[1500:], [16,50,84]),
                    c200,
                    np.percentile(fstd.q[1500:], [16,50,84]),
                    np.percentile(fstd.q2h[1500:], [16,50,84]),
                    np.percentile(fred.q[1500:], [16,50,84]),
                    np.percentile(fred.q2h[1500:], [16,50,84])]
    
            
            fq=open(folder+'../'+'res_woc_'+fname+'.out','a')
            fq.write(hsamples[j]+' & ')
        
            fq.write('$'+str('%.3f' % (qres[0][1]))+'_{-'+str('%.3f' % (np.diff(qres)[0][0]))+'}^{+'+str('%.3f' % (np.diff(qres)[0][1]))+'}$ & ')
            fq.write('$'+str('%.3f' % (qres[1]))+'$ & ')
            for x in qres[2:-1]:
                fq.write('$'+str('%.3f' % (x[1]))+'_{-'+str('%.3f' % (np.diff(x)[0]))+'}^{+'+str('%.3f' % (np.diff(x)[1]))+'}$ & ')
            x = qres[-1]
            fq.write('$'+str('%.3f' % (x[1]))+'_{-'+str('%.3f' % (np.diff(x)[0]))+'}^{+'+str('%.3f' % (np.diff(x)[1]))+r'}$ \\'+' \n')
        ...
        qEin  += [np.median(fstd.q[1500:])]
        eqEin += [np.diff(np.percentile(fstd.q[1500:], [16,50,84]))]
        
        if make_tab:
            fred = fits.open(folder+'fitresults_2h_2q_Ein_250_5000_reduced_profile_'+samp+'.fits')[1].data
            
            # '''
            qres = [np.percentile(fstd.lM200[1500:], [16,50,84]),
                    np.percentile(fstd.c200[1500:], [16,50,84]),
                    np.percentile(fstd.alpha[1500:], [16,50,84]),
                    np.percentile(fstd.q[1500:], [16,50,84]),
                    np.percentile(fstd.q2h[1500:], [16,50,84]),
                    np.percentile(fred.q[1500:], [16,50,84]),
                    np.percentile(fred.q2h[1500:], [16,50,84])]
            
            fq=open(folder+'../'+'res_Ein_'+fname+'.out','a')
            fq.write(hsamples[j]+' & ')
        ...
        qNFW  += [np.median(fstd.q[1500:])]
        eqNFW += [np.diff(np.percentile(fstd.q[1500:], [16,50,84]))]
        
        if make_tab:
            fred = fits.open(folder+'fitresults_2h_2q_250_5000_reduced_profile_'+samp+'.fits')[1].data
            ...
            fq=open(folder+'../'+'res_NFW_'+fname+'.out','a')
            fq.write(hsamples[j]+' & ')
    ...
    return qwm,q1h,eq1h,qwoc,eqwoc,qEin,eqEin,qNFW,eqNFW
...
res = np.loadtxt(folder+'../allres_NFW.tab').T
lMNFW    = res[0]
elMNFWm  = res[1]
elMNFWM  = res[2]
cNFW     = res[3]
...
# '''
qwm,qoq,eqoq,qwoc,eqwoc,qEin,eqEin,qNFW,eqNFW = bias_model(['HM_Lz_relaxed','LM_Lz_relaxed','HM_Hz_relaxed','LM_Hz_relaxed'])
...
for hs in range(4):
    for fp in range(4):
        plt.errorbar(fp+1+0.1*hs,fq[fp][0][hs]/qwm[hs],yerr=np.array([fq[fp][1][hs]/qwm[hs]]).T,fmt=cstyle[hs],markersize=10)

plt.legend(frameon = False)
plt.ylabel(r'$\tilde{q}_{1h}/q$')
ax.set_xticks(np.arange(4)+1)
ax.set_xticklabels(xl)
f.savefig(folder+'../final_plots/model_bias.pdf',bbox_inches='tight')
# '''
